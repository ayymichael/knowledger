    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant='ghost' className='p-px rounded-full self-center h-8 w-8'>
          {session.data ? <ProfileAvatar profile={user} className='h-8 w-8' /> : null}
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent className='w-56 mr-2 '>
        {session.data ? (
          <DropdownMenuLabel>
            <p>{user ? getProfileDisplayName(user) : undefined}</p>
            <p className='text-xs text-muted-foreground overflow-hidden text-ellipsis'>{session.data.user?.email}</p>
          </DropdownMenuLabel>
        ) : null}
